...
* **AVVMGameModeAdditive:** A modular UObject extension for AGameMode that dynamically alters runtime match behavior, extending the core game loop (ReadyToStartMatch/StartMatch/EndMatch) with transient gameplay systems. By decoupling these game rules from the main game mode class, developers can inject temporary features (like loading Game Feature Plugins or registering actor components) and hook into latent Blueprint gameplay events. Additionally, it supports isolated, decoupled testing by allowing developers to inject specific additive configurations via command-line arguments (using GameModeOptions={AssetName} with the naming convention BP_{ProjectName}_{AssetName}), which are automatically parsed by the system during server startup.
