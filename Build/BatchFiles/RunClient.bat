...
set "LOG_FILE=%CD%\launch_errors_client.log"
set "ENGINE_ROOT=%~dp0..\..\..\.."
...
if /i "%~1"=="clean" (
	...
	"%UBT_EXE%" -projectfiles -project="%UPROJECT%" -game -rocket -progress
...
"%UBT_EXE%" "%PROJECT_NAME%Client" Win64 Development -Project="%UPROJECT%" -WaitMutex
...
echo %TIMESTAMP% Launching Unreal Editor as Client...
:: 4. Launch and WAIT for the exit code, redirecting command-line errors to the log
start /wait "%UNREALED%" "%UPROJECT%" "127.0.0.1:7777" -game -windowed -ResX=640 -ResY=400 -log 2>> "%LOG_FILE%"
